#!/usr/bin/env bash
# Copyright (c) 2016 The Gthpcoin Core developers
# Distributed under the MIT software license, see the accompanying
# file COPYING or http://www.opensource.org/licenses/mit-license.php.

###   This script attempts to download the signature file SHA256SUMS.asc from
###   gthpcoincore.org and gthpcoin.org and compares them.
...
WORKINGDIR="/tmp/gthpcoin_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://gthpcoincore.org"
HOST2="https://gthpcoin.org"
BASEDIR="/bin/"
VERSIONPREFIX="gthpcoin-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'gthpcoin-',
...
if [ $? -ne 0 ]; then
   echo "gthpcoin.org failed to provide signature file, but gthpcoincore.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "gthpcoin.org and gthpcoincore.org signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Gthpcoin Core binary release signing key installed?"
   fi
