...
const char * const GTHPCOIN_CONF_FILENAME = "gthpcoin.conf";
...
public:
    ...
    static inline std::pair<bool,std::string> GetArg(const ArgsManager &am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "gthpcoind -foo=bar
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Gthpcoin
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Gthpcoin
    // Mac: ~/Library/Application Support/Gthpcoin
    // Unix: ~/.gthpcoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Gthpcoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Gthpcoin";
#else
    // Unix
    return pathRet / ".gthpcoin";
#endif
#endif
}
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    const std::string confPath = GetArg("-conf", GTHPCOIN_CONF_FILENAME);
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    // Check for untranslated substitution to make sure Gthpcoin Core copyright is not removed by accident
    if (strprintf(COPYRIGHT_HOLDERS, COPYRIGHT_HOLDERS_SUBSTITUTION).find("Gthpcoin Core") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + "The Gthpcoin Core developers";
    }
