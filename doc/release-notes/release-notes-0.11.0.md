Gthpcoin Core version 0.11.0 is now available from:

  <https://gthpcoin.org/bin/gthpcoin-core-0.11.0/>
...
For example, add the following to `gthpcoin.conf`:
...
related to the blockchain in the gthpcoin system: the raw blocks as received over 
the network (blk???.dat), the undo data (rev???.dat), the block index and the 
UTXO set (both LevelDB databases). The databases are built from the raw data.

Block pruning allows Gthpcoin Core to delete the raw block and undo data once 
...
block index and UTXO databases. The minimum was chosen so that Gthpcoin Core will 
...
For an example script see [gthpcoin-submittx](https://github.com/laanwj/gthpcoin-submittx).
...
- #5334 `cf87536` libgthpcoinconsensus: Add pkg-config support
...
- #6269 `95aca44` gitian: Use the new gthpcoin-detached-sigs git repo for OSX signatures
...
- #5729 `9c4a5a5` Allow unit changes for read-only GthpcoinAmountField
- #5753 `0f44672` Add gthpcoin logo to about screen
...
- #5809 `46bfbe7` Add gthpcoin-cli man page
...
- #5849 `41259ca` contrib/init/gthpcoind.openrc: Compatibility with previous OpenRC init script variables
...
- #6116 `30dc3c1` [OSX] rename Gthpcoin-Qt.app to Gthpcoin-Core.app
- #6168 `b3024f0` contrib/linearize: Support linearization of testnet blocks
- #6098 `7708fcd` Update Windows resource files (and add one for gthpcoin-tx)
